...
class Alimama:
    ...
    def sync_order(self):
        ...
        while True:
            page_no = 1
            self.logger.debug("=====start sync order=====")
            url = "{url}?r={r}&t={t}&_tb_token={tb_token}&jumpType={jump_type}&positionIndex={pos}&pageNo={page_no}&" \
                  "startTime={start_time}&endTime={end_time}&queryType={query_type}&tkStatus={tk_status}&" \
                  "memberType={member_type}&pageSize={page_size}" \
                .format(url=base_url, r='mx104', t=int(time.time() * 1000), tb_token=self.get_tb_token(), jump_type=0,
                        pos='', page_no=page_no, start_time=cn_date(), end_time=yesterday(), query_type=2, tk_status='',
                        member_type='', page_size=40)
